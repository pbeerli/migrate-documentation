...
\item{\bt{Genealogies}:} Reads the {\tt bayesallfile}   (see INPUT/OUTPUT section) of a previous runs,
...
\item{\bt{categories=$<$Yes $|$ No$>$}}\\
 If you specify {\bt{Yes}} you need a file named "{\tt catfile} in the same  directory
...
If you specify {\bt{Yes}} you need a file {\tt weightfile} with weights for each
...
%
%If you specify {\bt TREE} you need a file {\tt intree}. In this file you have
...
%{\tt A.A A.B C.?} but also {\tt 128.132 Rvs.Sahss}\\
...
the program will use the system clock. With {\bt{seed=Noauto}} the program expects to find a file named {\tt seedfile} with the random number seed.
...
Print the data in the {\tt outfile}. defaults is {\bt{print-data=No}}. If you run your data for the first time trhough \migrate turn this option on, because it helps to find problems with data-reading. Especially with microsatellite data it is possible that the program runs but the loci are incorrectly read.
...
\item{{\bt{None}}:} {\tt treefile} is not initialized and no trees are printed, 
...
The \bt{migration matrix} is a $n$ by $n$ table with {\bf -} on the diagonal and can look 
...
%{\it Migrate} is using the ${\rm F_{ST}}$ calculation only to generate 
...
\item{\bt{Number of bins of posterior ({\bf bayes-posteriorbins=$<$thetabins Mbins $<$ratebins$>$$>$)}}}\\
...
\item{\bt{Plotting bins of posterior ({\bf bayes-posteriormaxtype=$<$TOTAL $|$ P100$|$ P99$|$ MAXP99 $>$)}}}\\
...
\end{verbatim}
\end{small}
\item{\bt{Number of long chains to run? ({\bf long-chains=$<$value$>$)}}}\\
...
\item{\bt{Sampling increment? ({\bf long-inc=$<$value$>$)}}}\\
...
lab-jargons like {\tt Rvf} or {\tt sahss} ({\it Rana ridibunda} very fast, {\it Rana saharica} super slow)
